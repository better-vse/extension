...
//             const href = `https://insis.vse.cz/auth/student/pruchod_studiem.pl?vyber=podrobne_vsechna_obdobi;studium=214751;obdobi=2804`;
//             prehled = await fetchHtml(href);
//             $(document.body).append(`<div class="fak" id="newWrapper" style="display:none"></div>`)
//             $(`#fakultaHelperWrapper${facultyId}`).html(prehled)
//             $(".UISTMNumberCellHidden").remove()
//         };
//     }

//     let prehledPredmetuURL = $(`#newWrapper tr:nth-child(${$(`${predmetFacultyId}`).find(`tr:contains('${normalCode}')`).index() + 1}) img`).closest("a").attr("href");
